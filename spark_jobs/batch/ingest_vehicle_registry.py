...
=====================================
Reads vehicle_registry.csv from S3 landing zone, performs advanced data quality 
...
- Data Quality rules preventing future manufacturing years or unreasonable ones.
- Strict schema validation before enforcing schema with Spark.
- Structured logging.
...
logger = logging.getLogger("ingest_vehicle_registry")
...
EXPECTED_SCHEMA = StructType([
    ...
    StructField("baseline_kmpl", FloatType(), True),
])
...
INGESTED_PATH = os.environ.get("INGESTED_PATH", "s3a://ttn-de-bootcamp-bronze-us-east-1/poc-bootcamp-group5-bronze/ingested/") + "vehicle_registry"
...
def run(spark: SparkSession, run_date: str):
    ...
    current_year = datetime.now().year + 1  # allow +1 year for upcoming models
    valid_df = df.filter(
        col("vin").isNotNull() &
        (col("mfg_year").isNull() | ((col("mfg_year") >= 1950) & (col("mfg_year") <= current_year))) &
        (col("baseline_kmpl").isNull() | (col("baseline_kmpl") > 0))
    )
    ...
    # 6. Write Valid Data to Parquet (Overwrite specific partition to act as a Snapshot History)
    if valid_count > 0:
    ...
    if invalid_count > 0:
        quarantine_parquet = f"{QUARANTINE_PATH}/vehicle_registry"
    ...
    # 8. Emit DQ Metrics
    job_name = "ingest_vehicle_registry"
